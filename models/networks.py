...
import numpy as np
from .PCTransformer_block import PCFormer
import argparse
import torch
import torch.nn as nn
import torch.nn.functional as F
...
parser.add_argument('--depths', default=[2,4,4,2],  help='network_depth')
parser.add_argument('--embed_dims', default=[256,256,256,256],  help='embeddings')
parser.add_argument('--mlp_ratios', action='store_false', default=[2,4,4,2], help='mlp')
parser.add_argument('--num_heads', default=[4,8,8,4],  help='multi-head')
parser.add_argument('--attn_ratio', default=[1/2,1/2,1/2,1/2], help='attn_ratio')
parser.add_argument('--conv_type', default=['DWConv', 'DWConv', 'DWConv', 'DWConv'], type=str, help='path to logs')
parser.add_argument('--norm_layer', default=[RLN, RLN, RLN, RLN], type=str, help='path to logs')
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = TransGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=4, opt=opt)
...
class TransGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model += [PCFormer(network_depth=sum(args.depths), dim=args.embed_dims[i], depth=args.depths[i],
                    num_heads=args.num_heads[i], mlp_ratio=args.mlp_ratios[i],norm_layer=args.norm_layer[i], window_size=8,
                    attn_ratio=args.attn_ratio[i], attn_loc='last', conv_type=args.conv_type[i])
            ]
